...
We were interested in investigating where people were starting their journey from when visiting destinations around Manhattan. To do this, we:

1) Filtered our data to only observe rides that began and ended at different station 

2) Determined the 16 Citi Bike stations closest to 16 Manhattan destinations

3) Summarized data to find the number of trips that begin at one station and end at the Citi Bike station closest to the Manhattan destination

4) Filtered data to select the most common trip that end at each of the 16 Citi Bike stations (in the event of a tie, select the station that's furthest away)

5) Determined the average age 
...
selectInput(
  "site_choice", 
  label = h3("Select your destination:"), 
  choices = nyc_choices
)
```
...
### Most Common Bike Journey to Your NYC Destination
...
### Average Age of Riders at Destination Compared to that of Other Destinations
