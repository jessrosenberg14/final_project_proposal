...
## Using Plotly to Plot Ridership Data Acorss Multiple Transport Systems in NYC 
...
## Shiny and Leaflet - NYC Subway Across 2019-2020 
```{r}
# create color pallet 
pal <- colorNumeric(palette = "viridis",
                    domain = turnstiles_19_20_m$exits)

# create labels 
labs <- lapply(seq(nrow(turnstiles_19_20_m)), function(i) {
  paste0( '<p> Station Name: ', turnstiles_19_20_m[i, "stop_name"], '<p></p> Subway Lines: ', turnstiles_19_20_m[i, "daytime_routes"])
})
...
ui <- fluidPage(
  ...
  sliderInput(
    ...
    value  = as.Date("2019-01-01","%Y-%m-%d"),
    timeFormat = "%Y-%m-%d",
    step = 1,
...
# messing around trying to add subway lines 
subway_lines = geojsonio::geojson_read("subway_lines.geojson", what = "sp")

leaflet(subway_lines) %>%
